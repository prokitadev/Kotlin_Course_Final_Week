...
object RandomGame2048Initializer: Game2048Initializer<Int> {
    ...
    override fun nextValue(board: GameBoard<Int?>): Pair<Cell, Int>? {
        val returnInt = generateRandomStartValue()
        val nullCells = mutableListOf<Cell>()

        for (cell in board.getAllCells()) {
            val value = board.get(cell)
            if (value == null || value == 0) nullCells.add(cell)
        }
        if (nullCells.size == 0)  return null
        val randomCell = Random.nextInt(nullCells.size)
        return Pair(nullCells[randomCell], returnInt)
    }
}